...
/** PFix is a partially defined fixpoint combinator.
  * The above "fixpoint combinator" means, `PFix(h).toPartialFunction = h(PFix(h).toPartialFunction)` holds for any `h`.
  *
...
final class PFix[A, B] private (private val fs: Vector[(A => B) => PartialFunction[A, B]]) {
  ...
  /** Converts this into a total function.
    *
    * The result function handles a value as using this instance as possible.
    * If this instance cannot handle the value, then it passes to the fallback function.
    */
  def toFunction(fallback: A => B): A => B = new PFixFunction[A, B](fs, fallback)

  /** Converts this into a partial function.
    *
    * This method is almost same as `toFunction(_ => throw new MatchError)`,
    * but the result implements `PartialFunction` correctly.
    */
  ...
  /** Returns a new instance combined with this and that instances.
    *
    * The result instance handles a value as using `this` instance as possible.
    * If this instance cannot handle the value, then it tries to use `that` instance instead.
    */
...
object PFix {
  ...
  /** Creates a new instance from a higher-order function.
    *
    * The specified function takes a total function to call a future result function recursively,
    * and returns a partial function handles a value.
    */
